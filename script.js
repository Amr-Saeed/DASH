document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderCalendar() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            // Add event circles for the day
            const events = JSON.parse(localStorage.getItem("events")) || [];
            const eventOnDay = events.some(event => new Date(event.date).getDate() === day && new Date(event.date).getMonth() === currentMonth);
...
function getClosestElement(y) {
    const draggableItems = [...container.querySelectorAll('.widget-tasks, .widget-notes, .widget-calendar:not(.dragging)')];
